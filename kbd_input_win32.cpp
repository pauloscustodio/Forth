...
static uint32_t map_virtual_key(WORD vk) {
    ...
    case VK_DELETE:
        return KEY_DELETE;
    default:
        return vk; // fallback to raw VK code
    }
}
...
uint32_t get_ekey() {
    ...
    while (true) {
        ...
        if (input.EventType == KEY_EVENT && input.Event.KeyEvent.bKeyDown) {
            ...
            if (key.dwControlKeyState & (LEFT_CTRL_PRESSED | RIGHT_CTRL_PRESSED)) modifiers
                |= MOD_CTRL;
            if (key.dwControlKeyState & (LEFT_ALT_PRESSED | RIGHT_ALT_PRESSED))   modifiers
                |= MOD_ALT;
