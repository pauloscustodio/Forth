...
FACILITY EXT:
  Implemented:
    +FIELD BEGIN-STRUCTURE END-STRUCTURE
  Missing:
    CFIELD: EKEY EKEY>CHAR EKEY>FKEY EKEY? EMIT? FIELD: K-ALT-MASK
    K-CTRL-MASK K-DELETE K-DOWN K-END K-F1 K-F10 K-F11 K-F12 K-F2 K-F3 K-F4
    K-F5 K-F6 K-F7 K-F8 K-F9 K-HOME K-INSERT K-LEFT K-NEXT K-PRIOR K-RIGHT
    K-SHIFT-MASK K-UP MS TIME&DATE
...
NOT STANDARD:
  Implemented:
    #IN #TIB -2ROT -ROT .RS 0<= 0>= <= >= CONVERT D0<= D0<> D0> D0>= D<=
    D<> D> D>= DPL DU<= DU> DU>= EXPECT INTERPRET LATEST NUMBER NUMBER? OFF
