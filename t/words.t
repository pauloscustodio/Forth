...
forth_ok("words", <<'END');
/STRING TRAVERSE-WORDLIST SYNONYM NAME>INTERPRET NAME>STRING NAME>COMPILE >NAME
FORGET NR> N>R CS-ROLL CS-PICK AHEAD OFF ON SEE DUMP ENVIRONMENT? WORDS .FS .RS
.S RESIZE FREE ALLOCATE {: LOCALS| (LOCAL) SET-PRECISION PRECISION F~ FTRUNC
FSQRT FLNP1 FEXPM1 FLN FEXP FALOG FLOG FSINCOS FATAN2 FATANH FACOSH FASINH
FATAN FACOS FASIN FTANH FCOSH FSINH FTAN FCOS FSIN FABS F>S S>F FS. FE. F. F**
REPRESENT FROUND FNEGATE FMIN FMAX FLOOR SFLOATS SFLOAT+ DFLOATS DFLOAT+ FLOATS
FLOAT+ FDEPTH -FROT FROT FOVER FDUP FSWAP FDROP SFALIGNED SFALIGN DFALIGNED
DFALIGN FALIGNED FALIGN F0>= F0<= F0> F0< F0<> F0= F>= F<= F> F< F<> F= F/ F-
F* F+ SF@ SF! DF@ DF! F@ F! F>D D>F >FLOAT FVARIABLE FCONSTANT FLITERAL
...
D>= D> D<= D< D<> D= M+ M*/ D2/ D2* D- D+ 2LITERAL 2VARIABLE 2CONSTANT THRU
LIST UPDATE LOAD FLUSH EMPTY-BUFFERS SAVE-BUFFERS BUFFER BLOCK SCR BLK BYE QUIT
...
COMPILE, IMMEDIATE POSTPONE DOES> LITERAL CONSTANT TO FVALUE 2VALUE VALUE
BUFFER: VARIABLE CREATE ['] ' ] [ ; :NONAME : STATE EXIT EXECUTE EVALUATE
