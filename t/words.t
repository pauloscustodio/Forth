...
CONSTANT TO VALUE 2VARIABLE VARIABLE CREATE ['] ' ] [ ; :NONAME : STATE EXIT
EXECUTE EVALUATE INTERPRET TRACE U.R .R U. D.R D. . #> SIGN HOLD #S # <# SPACES
SPACE CR EMIT TYPE RESTORE-INPUT SAVE-INPUT QUERY EXPECT SPAN KEY ACCEPT REFILL
SOURCE-ID #TIB TIB SOURCE #IN >IN .( C" S" ." COUNT CONVERT >NUMBER NUMBER
NUMBER? DPL [CHAR] CHAR PARSE-WORD PARSE WORD MARKER UNUSED ALLOT ALIGNED ALIGN
>BODY FIND LATEST HERE C, , RSP! RSP@ R0 RDROP 2R@ 2R> 2>R J I R@ R> >R SP! SP@
S0 -2ROT 2ROT 2OVER 2DUP 2SWAP 2DROP TUCK ROLL PICK NIP DEPTH -ROT ROT OVER
?DUP DUP SWAP DROP MOVE ERASE FILL 2@ 2! C@ C! +! @ ! RSHIFT LSHIFT INVERT XOR
OR AND 0>= 0<= 0> 0< 0<> 0= U>= U<= U> U< >= <= > < <> = WITHIN CELLS CELL+
