...
forth_ok("words", <<'END');
INCLUDED DABS SEE DUMP ENVIRONMENT? WORDS .RS .S ABORT" ABORT BYE QUIT ENDCASE
...
IMMEDIATE POSTPONE DOES> 2LITERAL LITERAL 2CONSTANT CONSTANT TO VALUE 2VARIABLE
BUFFER: VARIABLE CREATE ['] ' ] [ ; :NONAME : STATE EXIT EXECUTE EVALUATE
INTERPRET TRACE U.R .R U. D.R D. . #> SIGN HOLDS HOLD #S # <# SPACES SPACE CR
EMIT TYPE RESTORE-INPUT SAVE-INPUT QUERY EXPECT SPAN KEY ACCEPT REFILL
...
-ROT ROT OVER ?DUP DUP SWAP DROP MOVE ERASE FILL 2@ 2! C@ C! +! @ ! RSHIFT
LSHIFT INVERT XOR OR AND 0>= 0<= 0> 0< 0<> 0= U>= U<= U> U< >= <= > < <> =
WITHIN CELLS CELL+ CHARS CHAR+ MIN MAX ABS UM* S>D NEGATE 2/ 2* 1- 1+ M* SM/REM
UM/MOD FM/MOD */MOD */ /MOD MOD / - * + HEX DECIMAL BASE TRUE FALSE PAD BL
END
