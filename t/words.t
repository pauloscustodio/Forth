...
forth_ok("words", <<'END');
/STRING RESIZE FREE ALLOCATE {: LOCALS| (LOCAL) SET-PRECISION PRECISION F~
FTRUNC FSQRT FLNP1 FEXPM1 FLN FEXP FALOG FLOG FSINCOS FATAN2 FATANH FACOSH
FASINH FATAN FACOS FASIN FTANH FCOSH FSINH FTAN FCOS FSIN FABS F>S S>F FS. FE.
F. F** REPRESENT FROUND FNEGATE FMIN FMAX FLOOR SFLOATS SFLOAT+ DFLOATS DFLOAT+
FLOATS FLOAT+ FDEPTH -FROT FROT FOVER FDUP FSWAP FDROP SFALIGNED SFALIGN
DFALIGNED DFALIGN FALIGNED FALIGN F0>= F0<= F0> F0< F0<> F0= F>= F<= F> F< F<>
F= F/ F- F* F+ SF@ SF! DF@ DF! F@ F! F>D D>F >FLOAT FVARIABLE FCONSTANT
FLITERAL FS-EXECUTABLE FS-WRITABLE FS-READABLE FS-SYMLINK FS-DIRECTORY
FS-REGULAR FS-EXISTS FILE-STATUS REQUIRED REQUIRE INCLUDE INCLUDE-FILE INCLUDED
...
EKEY>FKEY EKEY>CHAR EKEY EKEY? KEY KEY? END-STRUCTURE DFFIELD: SFFIELD: FFIELD:
2FIELD: FIELD: CFIELD: +FIELD BEGIN-STRUCTURE PAGE AT-XY ABORT" ABORT CATCH
THROW DNEGATE DMIN DMAX DABS D>S D0>= D0> D0<= D0< D0<> D0= DU>= DU> DU<= DU<
D>= D> D<= D< D<> D= M+ M*/ D2/ D2* D- D+ 2LITERAL 2VARIABLE 2CONSTANT OFF ON
SEE DUMP ENVIRONMENT? WORDS .FS .RS .S THRU LIST UPDATE LOAD FLUSH
